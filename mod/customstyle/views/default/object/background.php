...
$creditline = "";
	...
	if(get_plugin_setting("allowUploadBackground","customstyle") != "no"){
		...
		if($filehandler->exists()){
			$imageUrl = 'mod/customstyle/getbackground?id=' . $current_user;
			$imageUrl = elgg_add_action_tokens_to_url("$imageUrl");
			if($imageUrl != $currentConfig["background-image"]){
				$cust .= "<td>";
				$cust .= elgg_view('input/securitytoken');
				$cust .= "<input type='radio' name='background-image' value='" . $imageUrl . "' onclick='document.body.style.backgroundImage = \"url(" . $CONFIG->wwwroot . $imageUrl . ")\"'> " . elgg_echo("customstyle:background:previouslyuploadedbackground") . "<br />";
				$cust .= "<a  class='box' href='" . $CONFIG->wwwroot . $imageUrl . "' title='" . elgg_echo('customstyle:background:previouslyuploadedbackground') . "'><img src='" . $CONFIG->wwwroot . $imageUrl . "&thumb=true' style='width:150px;' alt='" . elgg_echo('customstyle:background:previouslyuploadedbackground') . "'/></a>";
				$cust .= "</td>";
				$i++;
			}
		}
...
$creditline = "";
	...
	if(get_plugin_setting("allowUploadBackground","customstyle") != "no"){
		$cust .= "<input type='radio' id='custombackground' name='background-image' value='custombackground'> " . elgg_echo("customstyle:background:custombackground") . "<br />";
		...
		$cust .= "<input type='hidden' name='MAX_FILE_SIZE' value='" . $max_file_size . "' />";
		$cust .= elgg_view("input/file",array("internalname"=>"backgroundfile", "js"=>"onclick=\"$('#custombackground').attr('checked', 'checked');\"")) . "<br /><br />";
	}
